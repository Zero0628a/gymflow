...
\begin{itemize}
    \item Implementar un sistema de registro e inicio de sesión para los usuarios de la aplicación, incluyendo el acceso con cuenta de Google.
    \item Personalizar la experiencia del usuario mediante un proceso de configuración inicial que recoja su objetivo, nivel, días disponibles y equipamiento.
    ...
    \item Permitir la creación, edición y gestión de rutinas de entrenamiento personalizadas.
    \item Ofrecer rutinas prediseñadas que se ajusten al perfil del usuario.
    \item Planificar y ejecutar las sesiones de entrenamiento dentro de un calendario semanal, registrando series, peso, repeticiones y esfuerzo percibido (RPE).
    \item Registrar y consultar el historial detallado de entrenamientos realizados por el usuario.
    \item Garantizar el funcionamiento de la aplicación sin conexión a internet, sincronizando la información al recuperar la red.
...
\begin{itemize}
    ...
    \item El sistema deberá permitir a los usuarios registrados iniciar sesión en la aplicación mediante un mecanismo de autenticación segura, incluyendo el acceso con cuenta de Google.

    \item El sistema deberá guiar al usuario nuevo a través de un proceso de configuración inicial que registre su objetivo, nivel, días disponibles y equipamiento, y le sugiera una rutina acorde.
    ...
    \item El sistema deberá organizar las rutinas activas en un calendario semanal, distribuyendo las sesiones según los días disponibles del usuario y diferenciando los días de entrenamiento de los de descanso.

    \item El sistema deberá permitir ejecutar la sesión del día marcando series y ejercicios, y registrar el peso, las repeticiones y el esfuerzo percibido (RPE) de cada serie.

    \item El sistema deberá permitir adaptar la sesión en curso reemplazando un ejercicio por una alternativa o posponiendo el día de entrenamiento.

    \item El sistema deberá permitir visualizar un historial detallado de los entrenamientos realizados, incluyendo el estado de cada sesión y las series registradas.

    \item El sistema deberá mantener disponible la información principal del usuario sin conexión a internet y sincronizarla al recuperar la red.
